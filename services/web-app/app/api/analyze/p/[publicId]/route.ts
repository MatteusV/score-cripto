...
export async function GET(
  ...
  try {
    ...
    const { requestId, ...rest } = data as {
      requestId?: string;
      [k: string]: unknown;
    };
    const normalized: Record<string, unknown> = {
      ...rest,
      processId: requestId ?? rest.processId,
    };
    ...
    if (
      ...
      logger.info("historical analysis result translated", {
        publicId,
        locale,
      });
      return NextResponse.json({
        ...normalized,
        result: translated ? { ...result, ...translated } : result,
      });
    }
