...
export default function BillingPage() {
  ...
  const { user, isPro, analysisCount, analysisLimit, analysisRemaining } = useUser()
  ...
  const periodLabel = cycle === "annual" ? t("plans.periodAnnual") : t("plans.period")
    ...
    () => [
      {
        ...
        cta: !isPro ? t("plans.current") : t("plans.startFree"),
      },
      {
        id: "pro" as const,
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col gap-6 px-6 pt-6 pb-12 lg:px-7">
          ...
          <div className="relative grid gap-7 p-7 lg:grid-cols-[1.4fr_1fr] lg:p-8">
            <div>
              <div className="mb-3 flex flex-wrap items-center gap-2.5">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/30 bg-primary/10 px-2.5 py-0.5 text-[10px] font-bold tracking-wider text-primary uppercase">
                  ...
                  {loadingSub
                    ...
                    : subscription?.status === "active"
                      ...
                      : isPro
                        ? t("currentPlan.active")
                        : "—"}
                  {renewDate && subscription?.status === "active" ? ` · ${t("currentPlan.renewsIn", { date: renewDate })}` : ""}
                </span>
              </div>
              <div className="mb-1.5 flex items-baseline gap-3">
                ...
                <span className="font-heading text-2xl font-bold text-primary">
                  ...
                  {isPro && (
                    <span className="ml-1 text-[13px] font-normal text-muted-foreground">{t("plans.period")}</span>
                  )}
                </span>
              </div>
              <p
                className="mt-2 mb-5 max-w-lg text-sm leading-relaxed text-muted-foreground"
                dangerouslySetInnerHTML={{
                  __html: t.markup("hero.remaining", {
                    remaining: String(analysisRemaining),
                    strong: (chunks) => `<strong class="text-foreground">${chunks}</strong>`,
                  }),
                }}
              />
              <div className="flex flex-wrap gap-2">
                {isPro ? (
                  ...
                  >
                    ...
                    {redirecting === "checkout" ? t("upgradingStripe") : t("hero.upgradeTeam")}
                  </Button>
                )}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-card/40 p-5">
              <div className="mb-3.5 flex items-center justify-between">
                ...
                {renewDate && (
                  <span className="font-mono text-[10px] text-muted-foreground">{renewDate}</span>
                )}
              </div>
              <div>
                <div className="mb-1 flex items-baseline justify-between text-[11.5px]">
                  ...
                  <span className="font-mono text-foreground">
                    {analysisCount}
                    <span className="text-muted-foreground">/{analysisLimit || "—"}</span>
                  </span>
                </div>
                <div className="h-1 overflow-hidden rounded-full bg-foreground/[0.06]">
                  <div
                    ...
                    style={{
                      width: analysisLimit > 0 ? `${(analysisCount / analysisLimit) * 100}%` : "0%",
                    }}
                  />
                </div>
              </div>
        ...
        <section>
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="m-0 font-heading text-2xl font-bold tracking-wide">
                {t("cycle.headline")}
              </h2>
            </div>
            <div className="flex items-center gap-2.5">
              <div className="flex gap-0.5 rounded-xl border border-border bg-muted p-0.5">
                {([
                  ...
                  <button
                    ...
                    onClick={() => setCycle(v)}
                    className={cn(
                      "cursor-pointer rounded-lg px-3.5 py-1.5 font-heading text-[11px] font-bold tracking-wider uppercase transition-colors",
                      cycle === v
              ...
              {cycle === "annual" && (
                <span className="rounded-full border border-primary/30 bg-primary/10 px-2.5 py-0.5 text-[10px] font-bold tracking-wider text-primary uppercase">
                  {t("cycle.save")}
                </span>
              )}
            </div>
          ...
          {cycle === "annual" && (
            ...
            <p className="mb-3 font-mono text-[11px] text-muted-foreground">{t("cycle.comingSoon")}</p>
          )}
            ...
            {plans.map((p) => (
              <article
                ...
                className={cn(
                  ...
                  p.featured ? "border-primary/30 bg-primary/[0.06] shadow-[0_20px_60px_-30px_oklch(0.74_0.19_66/40%)]" : `${p.ring} ${p.bg}`,
                )}
              >
                {p.featured && (
                  <div
                    ...
                    style={{
                      background: "radial-gradient(circle, oklch(0.74 0.19 66 / 30%), transparent 70%)",
                    }}
                  />
                )}
                {p.current && (
                  <span className="absolute top-4 right-4 rounded-full border border-primary/30 bg-primary/10 px-2 py-0.5 text-[9px] font-bold tracking-wider text-primary uppercase">
                    {t("plans.atualBadge")}
                  </span>
                )}
                {p.comingSoon && (
                  <span className="absolute top-4 right-4 rounded-full border border-accent/40 bg-accent/10 px-2 py-0.5 text-[9px] font-bold tracking-wider text-accent uppercase">
                    {t("plans.soonBadge")}
                  </span>
                )}

                <p className={cn("mb-2.5 font-heading text-[10px] font-bold tracking-[0.3em] uppercase", p.accent)}>
                  {p.name}
                </p>
                <p className="mb-3.5 text-xs text-muted-foreground">{p.caption}</p>
                <div className="mb-1 flex items-baseline gap-1">
                  <span className="font-heading text-4xl leading-none font-bold text-foreground">
                    {p.priceLabel}
                  </span>
                  {p.amount > 0 && (
                ...
                <Button
                  ...
                  onClick={() => {
                    if (p.id === "pro" && !p.current) void handleCheckout()
                  }}
                  className={cn(
                ...
                <ul className="flex flex-col gap-2.5 border-t border-border pt-5">
                  {p.features.map((f, i) => (
                    ...
                    >
                      ...
                      <span className={cn("leading-snug", !f.ok && "line-through decoration-muted-foreground/40")}>
                        {f.label}
                      </span>
                    </li>
                  ))}
        ...
        <section className="grid gap-5 lg:grid-cols-[1.3fr_1fr]">
          <div className="rounded-2xl border border-border bg-card p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              {renewDate && (
                <span className="font-mono text-[10px] text-muted-foreground">
                  {t("usage.cycleRange", { start: t("currentPlan.active"), end: renewDate })}
                </span>
              )}
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              {[
                ...
                { label: t("usage.watchlist"), used: 0, cap: 0, color: "var(--accent)", locked: true },
                { label: t("usage.exports"), used: 0, cap: 0, color: "oklch(0.69 0.19 162)", locked: true },
                {
                  label: t("usage.api"),
                  used: 0,
                  cap: 0,
                ...
                >
                  <div className="mb-2.5 flex items-center justify-between">
                    <span className="text-[11.5px] font-medium text-foreground">{u.label}</span>
                    {u.locked && (
                      <span className="inline-flex items-center gap-1 rounded-full border border-border px-1.5 py-0.5 text-[9px] font-bold tracking-wider text-muted-foreground uppercase">
                        <LockIcon className="size-2.5" />
                        {t("usage.lockedTeam")}
                      </span>
                    )}
                  </div>
                  <div className="mb-2 font-heading text-[22px] leading-none font-bold" style={{ color: u.color as string }}>
                    {u.locked ? "—" : u.used}
                  ...
                  <div className="h-1 overflow-hidden rounded-full bg-foreground/[0.06]">
                    <div
                      ...
                      style={{
                        width: u.locked || u.cap === 0 ? "0%" : `${(u.used / u.cap) * 100}%`,
          ...
          <div className="rounded-2xl border border-border bg-card p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <SparklesIcon className="size-4 text-primary" strokeWidth={1.75} />
            </div>
            ...
            <dl className="flex flex-col gap-2 border-t border-border pt-4 text-xs">
              <div className="flex items-center justify-between">
                <dt className="text-muted-foreground">{t("manage.nextCharge")}</dt>
                <dd className="font-mono text-foreground">
                  {isPro && renewDate ? `${renewDate} · ${formatBRL(PLAN_PRICES.pro)}` : t("manage.fallback")}
                </dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-muted-foreground">{t("manage.billingEmail")}</dt>
                <dd className="font-mono text-[11px] text-foreground">{user?.email ?? t("manage.fallback")}</dd>
              </div>
              <div className="flex items-center justify-between">
                <dt className="text-muted-foreground">{t("manage.billingCountry")}</dt>
                <dd className="text-foreground">{isPro ? "Brasil" : t("manage.fallback")}</dd>
              </div>
            </dl>
            {!isPro && (
        ...
        {/* ── FAQ ─────────────────────────────────────────────────────── */}
        <section>
          <p className="mb-3.5 font-heading text-[10px] font-bold tracking-[0.3em] text-muted-foreground uppercase">
            {t("faq.eyebrow")}
          </p>
          <div className="grid gap-3 lg:grid-cols-2">
            {(t.raw("faq.items") as [string, string][]).map(([q, a], i) => (
              ...
              >
                <p className="mb-1.5 text-[13px] font-semibold text-foreground">{q}</p>
